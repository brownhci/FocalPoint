...
public class LeftHandBoneCopying : MonoBehaviour {
        ...
        if (m_parent != null) {
            ...
            for (int i = 1; i < 5; ++i) {
                ...
                for (int j = 0; j < 2; ++j) {
                    ...
                    // Debug.Log("Counter:" + counter + " and " + m_selfFingers[counter + 1]);
                    counter++;
                }
                counter++;
            }
            ...
            // Debug.Log("Target:" + m_targetFingers[0]);
            counter = 1;
            for (int i = 0; i < 5; ++i) {
    ...
    void Update() {
        /*if (m_available) {
            m_selfFingers[0].localScale = Vector3.one;
            for (int i = 0; i < 16; ++i) {
                m_selfFingers[i].position = m_targetFingers[i].position;
                m_selfFingers[i].rotation = m_targetFingers[i].rotation;
                if (i == 0) {
                    m_selfFingers[0].Rotate(0f, 0f, -90f, Space.Self);
                    m_selfFingers[0].Rotate(90f, 0f, 0f, Space.Self);
                    m_selfFingers[0].position += m_selfFingers[0].right * 0.055f;
                }
                else {
                    m_selfFingers[i].Rotate(0f, 0f, 90f, Space.Self);
                    //m_selfFingers[i].Rotate(90f, 0f, 0f, Space.Self);
                    // palm is fine, but fingers need some constrain to get a natural effect
                    if (i % 3 != 1) {
                        m_selfFingers[i].position += (m_selfFingers[i - 1].right + m_selfFingers[i - 1].up) * 0.007f;
                    }
                }
            }
            m_selfFingers[0].localScale = new Vector3(m_sizeFactor, m_sizeFactor, m_sizeFactor);

...
public class PaintManager : MonoBehaviour {
    ...
    void setupPaintPlane() {
        ...
        if(!is2dPlanePlaced) {
            ...
            if(gestureManager.bufferedGesture() == "palm" || (Input.touchCount > 0 
                ...
                Debug.Log("touch PLANE IS PLACED");
    ...
    private bool checkTouchValidity() {
        ...
        Debug.Log("check Touched validity called");
        ...
        Debug.Log("transform parent is" + paintPlane.transform.parent);

        if (paintPlane.transform.parent == palm.transform) {
            Debug.Log("parent is palm");
            return false;
        }
	...
	public void turnOnPaint(){
        //if (paint_mode == true) {
        //    // if it's already turned on, toggle brush mode
	...
	private void createNewInk(){
        if (currentMaterial < 0 || currentMaterial >= materialPrototypes.Count) {
            Debug.LogError("Trying to use an illegal material to draw lines");
            return;
        }
    ...
    public void UpdateEndpoints() {
        ...
        endpoints_list = new List<Vector3> ();
        //Debug.Log("inklist is" + inkList.Count);

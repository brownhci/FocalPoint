...
public class HandManager : MonoBehaviour {
    ...
    private VirtualMenuBrush m_brushMenu;
		...
		if (context != set_to_context){
            if (set_to_context == "paint") {
                ...
                if (m_currentMenu == 0) {
                    ...
                    // if paintmenu open, then relocate
                    m_paintMenu.open();
					m_furnitureMenu.close ();
                    m_brushMenu.close();
                }
            ...
            if (m_currentMenu == 0) {
                m_furnitureMenu.setToCameraPosition();
            } else if (m_currentMenu == 3) {
                m_brushMenu.setToCameraPosition();
            } else {
                m_paintMenu.setToCameraPosition();
            }
        }
		return;
	}

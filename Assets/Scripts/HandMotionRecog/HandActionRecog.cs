...
public class HandActionRecog : MonoBehaviour {
	...
	public void LogAllList () {
		Debug.Log ("Start to log action list, total:" + leftHandList.Count);
		LinkedListNode<HandActionItem> lln = leftHandList.Last;
		while(lln != null && lln.Value != null) {
			Debug.Log ("Gesture:" + lln.Value.Gesture + " and PalmNorm:" + lln.Value.PalmNorm + " and time:" + lln.Value.InsertTime);
			lln = lln.Previous;
		}
		Debug.Log ("End to output list");
	}
